...
class MetaDbPollMetadataTest: public QObject
{
    Q_OBJECT
private Q_SLOTS:
...
void MetaDbPollMetadataTest::initTestCase(void)
{
...
void MetaDbPollMetadataTest::testPolling_data(void)
{
...
void MetaDbPollMetadataTest::testPolling(void)
{
    ...
    const auto& test = test::storage::db::createMetadataTest(
        entryId, [&entryId, hash, keyEncoding, tokenType, &message](QSharedPointer<otp::storage::db::MetadataDbManager>, QSharedPointer<otp::storage::db::Metadata> metadata) -> void
    {
...
QTEST_APPLESS_MAIN(MetaDbPollMetadataTest)

#include "metadb-poll-metadata.moc"
